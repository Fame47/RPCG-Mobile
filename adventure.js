(() => {
  ...
  const NODE_META = {
    ...
    strong:   { icon: '⚔', label: 'STRONG ENEMY' },
  ...
  const AREA_POOLS = {
    1: {
      ...
      cave: ['spider','goblin_scout','wolf'],
      dungeon: ['bandit','armored_goblin'],
      boss: ['goblin_king']
    },
    2: {
      normal: ['boar','spider','bandit','bandit_outrider','traveling_mercenaries','goblin_scout','armored_goblin'],
      strong: ['bandit_outrider','traveling_mercenaries','armored_goblin','berserk_goblin','owlbear'],
      elite: ['owlbear','berserk_goblin','traveling_mercenaries'],
      cave: ['spider','armored_goblin','berserk_goblin'],
      dungeon: ['armored_goblin','bandit_outrider','traveling_mercenaries'],
      boss: ['mirror_knight','mirror_warrior','goblin_king']
    },
    3: {
      normal: ['bandit_outrider','traveling_mercenaries','armored_goblin','berserk_goblin','owlbear'],
      strong: ['traveling_mercenaries','armored_goblin','berserk_goblin','owlbear'],
      elite: ['owlbear','berserk_goblin','mirror_warrior'],
      cave: ['spider','berserk_goblin','owlbear','armored_goblin'],
      dungeon: ['traveling_mercenaries','armored_goblin','berserk_goblin'],
      boss: ['mirror_mage','mirror_healer','goblin_king']
    }
  };

  const ROUTE_POSITIONS = [
    [50,89],[30,82],[63,77],[77,70],[48,65],
    [24,59],[43,53],[72,48],[79,41],[55,35],
    [30,30],[22,23],[47,18],[73,13],[56,7]
  ];
  ...
  function enemyForNode(areaNo, type) {
    const pool = AREA_POOLS[areaNo] || AREA_POOLS[1];
    const source = pool[type] || pool.normal;
    return pick(source);
  }

  function generateArea(areaNo) {
    const length = areaNo === 1 ? randInt(10, 12) : areaNo === 2 ? randInt(11, 13) : randInt(12, 15);
    const treasureCount = randInt(1, 2); // Always at least one true Treasure Room.
    const campCount = randInt(1, 2);
    const caveCount = areaNo === 1 ? randInt(0, 1) : areaNo === 2 ? 1 : randInt(1, 2);
    const eliteCount = areaNo === 3 ? 2 : 1;

    const firstChoices = areaNo === 3 ? ['enemy','enemy','strong','cave'] : ['enemy','enemy','strong'];
    const first = pick(firstChoices);
    const middleSlots = length - 3; // first + dungeon + boss are held outside this pool.
    const types = [];

    for (let i = 0; i < treasureCount; i += 1) types.push('treasure');
    for (let i = 0; i < campCount; i += 1) types.push('camp');
    for (let i = 0; i < caveCount; i += 1) types.push('cave');
    for (let i = 0; i < eliteCount; i += 1) types.push('elite');

    while (types.length < middleSlots) {
      types.push(Math.random() < .36 ? 'strong' : 'enemy');
    }
    if (types.length > middleSlots) types.length = middleSlots;

    let middle = shuffle(types);
    // Avoid turning the front half into a rest-stop parade.
    for (let i = 1; i < middle.length; i += 1) {
      if (middle[i] === 'camp' && middle[i - 1] === 'camp') {
        const swap = middle.findIndex((t, idx) => idx > i && !['camp','treasure'].includes(t));
        if (swap > i) [middle[i], middle[swap]] = [middle[swap], middle[i]];
      }
    }

    const sequence = [first, ...middle, 'dungeon', 'boss'];
    return sequence.map((type, index) => ({
      id: `a${areaNo}n${index + 1}`,
      type,
      enemyId: ['enemy','strong','elite','cave','dungeon','boss'].includes(type) ? enemyForNode(areaNo, type) : null,
      complete: false,
      visited: false,
      rewardTaken: false
    }));
  ...
  function createAdventure() {
    const state = {
      ...
      node: 0,
      startedAt: Date.now(),
      areas: [generateArea(1), generateArea(2), generateArea(3)]
    };
    saveAdventure(state);
    return state;
  }
  ...
  function currentNode(state) {
    return state?.areas?.[state.area]?.[state.node] || null;
  }
  ...
  function completeCurrentNode(state) {
    ...
    const areaNodes = state.areas[state.area];
    if (state.node >= areaNodes.length - 1) {
  ...
  function injectUI() {
    ...
    mode.innerHTML = `
      <div class="battle-mode-box" role="dialog" aria-modal="true" aria-label="Choose Battle Mode">
        ...
        <button id="adventureModeChoice" class="battle-mode-choice adventure-choice" type="button">
          <span>ADVENTURE MODE</span><small>Travel the realm through a randomized run.</small><b id="adventureModeProgress"></b>
        </button>
    ...
    screen.innerHTML = `
      ...
      <header class="adventure-map-header">
        ...
        <h1 id="adventureAreaTitle">AREA I</h1>
    ...
    complete.innerHTML = `
      <div class="adventure-room-box complete-box">
        ...
        <p>You cleared all three areas.</p>
  ...
  function showModeSelect() {
    ...
    if (state?.active && !state.completed) {
      ...
      progress.textContent = `AREA ${state.area + 1} • NODE ${state.node + 1}/${state.areas[state.area].length}`;
    } else {
      choice.querySelector('span').textContent = 'ADVENTURE MODE';
      progress.textContent = state?.completed ? 'START A NEW RUN' : 'NEW RANDOMIZED RUN';
    }
  ...
  function renderAdventureMap() {
    ...
    const areaNo = state.area + 1;
    const nodes = state.areas[state.area];
    const route = document.getElementById('adventureRoute');
    const mapArt = document.querySelector('.adventure-map-art');

    document.getElementById('adventureAreaTitle').textContent = `AREA ${['I','II','III'][state.area]}`;
    document.getElementById('adventureProgressText').textContent = `NODE ${state.node + 1} / ${nodes.length}`;
    mapArt.style.backgroundImage = `url('${MAP_ASSET}')`;
    mapArt.dataset.area = String(areaNo);
    ...
    if (node) {
      ...
      if (node.type === 'cave') detail = 'A cave encounter waits inside.';
      if (node.type === 'dungeon') detail = 'Enter the dungeon. The Area Boss waits beyond it.';
      if (node.type === 'boss') detail = 'Defeat the Area Boss to unlock the next map.';
  ...
  function enterCamp(state, node) {
    ...
    // Start the exact existing Healer timer automatically when a bed is free.
  ...
  function adventureLevelFor(node, areaNo, heroLevel) {
    if (node.type === 'enemy') return heroLevel + randInt(1, 3) + (areaNo - 1);
    if (node.type === 'strong') return heroLevel + randInt(3, 5) + (areaNo - 1);
    if (node.type === 'elite') return heroLevel + randInt(5, 7) + areaNo;
    if (node.type === 'cave') return heroLevel + randInt(3, 5) + areaNo;
    if (node.type === 'dungeon') return heroLevel + randInt(4, 6) + areaNo;
    if (node.type === 'boss') return heroLevel + 5 + (areaNo * 2);
    return heroLevel + 1;
  }
  ...
  function startAdventureBattle(state, node) {
    ...
    const base = cfg?.encounters?.[node.enemyId];
    if (base) {
      const areaNo = state.area + 1;
      const level = adventureLevelFor(node, areaNo, api.heroLevel?.() || battle.hero.level || 1);
      const scaled = api.scaleEncounterForLevel?.(base, level) || base;
      const displayPrefix = node.type === 'elite' ? 'Elite ' : node.type === 'strong' ? 'Veteran ' : '';
      Object.assign(battle.enemy, {
        id: base.id,
        name: `${displayPrefix}${base.name}`,
        kind: base.kind,
        image: base.image,
        level,
        isAvenger: false,
      ...
      syncForcedEnemyUI(battle, node, state);
    }

    setAdventureBattlePresentation(node, state);
  }

  function syncForcedEnemyUI(battle, node, state) {
    ...
    if (progress) progress.textContent = `ADVENTURE • AREA ${state.area + 1} • NODE ${state.node + 1}/${state.areas[state.area].length}`;
    splash?.classList.remove('show');
  }

  function setAdventureBattlePresentation(node, state) {
    ...
    if (badge && badgeText) {
      badge.hidden = false;
      badgeText.textContent = `AREA ${state.area + 1} • ${NODE_META[node.type].label}`;
    }
    ...
    if (escape) escape.textContent = 'WORLD MAP';
  }
  ...
  function restoreAdventureBattlePresentation() {
    ...
    const escape = document.getElementById('battleMenuBtn');
    if (escape) escape.textContent = 'ESCAPE';
  ...
  function handleAdventureBattleResult() {
    ...
    document.querySelector('#battleResult .result-card')?.classList.add('adventure-result-card');
  }
  ...
  function flashMapMessage(text) {
    ...
    setTimeout(() => { if (document.getElementById('adventureScreen')?.classList.contains('show')) { renderAdventureMap(); } else info.innerHTML = previous; }, 1700);
  }
  ...
  function wireUI() {
    ...
    // Capture Battle navigation before the original data-go handler. Endless War is still the untouched original battle code.
    document.addEventListener('click', event => {
      ...
      if (adventureBattle) {
        ...
        if (resultContinue && document.getElementById('battleResult')?.classList.contains('show')) {
          ...
          leaveAdventureBattleToMap(true);
          return;
        }
